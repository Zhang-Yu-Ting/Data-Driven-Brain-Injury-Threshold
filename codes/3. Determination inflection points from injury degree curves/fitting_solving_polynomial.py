...
def fitting_solving(data, thres_range, file_name1, file_name2, path1):
    ...
    p1 = np.polyfit(data[0], data[1], fitting_num)
        
    # Solve for complex roots and extract the real parts from them.
    x = sp.symbols('x')

    fit_func = 0
    for tt in range(fitting_num + 1):

        fit_func += p1[tt] * x ** (fitting_num - tt)
    ...
    for jj in range(fitting_num):
        d1_coefficient.append(p1[jj] * (fitting_num - jj))

    for jj in range(fitting_num - 1):
        d2_coefficient.append(p1[jj] * (fitting_num - jj) * (fitting_num - jj - 1))
    ...
    plt.tight_layout()
    plt.xticks()
    plt.savefig(path1 + '\\' + str(fitting_num) + '_th.jpg')
    plt.close()
    ...
    if x_solution == []:
        ...
        with open(file_name1, 'a') as datatmp:
            datatmp.writelines(str(fitting_num) + '   ' + 'no inflection point   ' + 'R^2 = ' + str(R_tmp) + '\n')
        ...
        # Record results.
        with open(file_name1, 'a') as datatmp:
            datatmp.writelines(str(fitting_num) + '   original_results_all\n')
    ...
    # Record the results for 5th degrees.
    with open(file_name2, 'a') as datatmp:
        datatmp.writelines('inflection_points   ' + str('%.4g' %point_all[0]) + '\n' )
        datatmp.writelines('              R^2   ' + str('%.4g' %R[0]) + '\n')
